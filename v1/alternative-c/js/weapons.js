const Weapons = (() => {
    ...
    function addWeapon(type) {
        ...
        inventory.push({
            ...
            spreadAngle: d.spreadAngle || 0, aoe: d.aoe || 0,
            selfDamage: d.selfDamage || false,
    ...
    function canFire(now) {
        const w = current();
        return w && !w.reloading && w.clip > 0 && now - w.lastFireTime >= w.fireRate;
    }

    function fire(now) {
        const w = current();
        if (!canFire(now)) return null;
        w.clip--; w.lastFireTime = now;
    ...
    function startReload(now) {
        ...
        w.reloading = true; w.reloadStartTime = now;
    }
    ...
    function updateReload(now) {
        ...
        if (now - w.reloadStartTime >= w.reloadTime) {
            const need = w.clipSize - w.clip;
            if (w.infiniteAmmo) w.clip = w.clipSize;
            else { const ld = Math.min(need, w.reserve); w.clip += ld; w.reserve -= ld; }
            w.reloading = false; return true;
        }
        return false;
    }
    ...
    function addAmmo(type, amt) {
        ...
        if (w && !w.infiniteAmmo) { w.reserve += amt; return true; }
        return false;
    }

    function getInventory() { return inventory; }
    return { reset, addWeapon, current, switchTo, canFire, fire, startReload, updateReload, addAmmo, getInventory };
})();
